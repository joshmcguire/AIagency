...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? 'bg-white/95 backdrop-blur-md border-b border-slate-200/50 shadow-lg' : 'bg-transparent'
    }`}>
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center">
            <div className="flex items-center space-x-3">
              ...
              }`}>Strategic AI Partners</span>
            </div>
          </div>

          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center space-x-8">
            <a href="#services" className={`font-medium transition-colors duration-200 ${
              isScrolled ? 'text-slate-700 hover:text-slate-900' : 'text-white hover:text-blue-200'
            }`}>
              Services
            </a>
            <a href="#methodology" className={`font-medium transition-colors duration-200 ${
              isScrolled ? 'text-slate-700 hover:text-slate-900' : 'text-white hover:text-blue-200'
            }`}>
              Methodology
            </a>
            ...
            <div className={`w-px h-6 ${isScrolled ? 'bg-slate-300' : 'bg-white/30'}`}></div>
            <a
              href="#contact"
              className="bg-blue-600 text-white px-6 py-2.5 rounded-md text-sm font-semibold hover:bg-blue-700 transition-all duration-300 shadow-lg hover:shadow-xl"
            >
      ...
      {isOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-md border-t border-slate-200">
          <div className="px-4 py-6 space-y-4">
            ...
            <a href="#methodology" className="block text-slate-700 hover:text-slate-900 font-medium py-2">
              Methodology
            </a>
            ...
            <a
              href="#contact"
              className="block bg-blue-600 text-white px-6 py-3 rounded-md text-sm font-semibold text-center mt-4 hover:bg-blue-700 transition-colors"
            >
              Schedule Consultation
            </a>
          </div>
